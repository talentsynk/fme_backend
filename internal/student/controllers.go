...
func GetStudent(c *gin.Context) {
	...
	case 1:
		err := config.DB.Table("students").
			Select("students.id AS id, students.firstname AS first_name, students.lastname AS last_name, students.phone_number AS phone_number, users.is_active AS is_active,  users.email AS email, users.id AS user_id, STRING_AGG(courses.name, ', ') AS courses_taken, students.gender AS gender, students.state_of_residence AS state_of_residence, students.address AS address, MAX(students.created_at) AS created_at, students.graduation_status AS is_graduated").
			Joins("JOIN users ON students.user_id = users.id").
			Joins("LEFT JOIN student_courses ON students.id = student_courses.student_id").
			Joins("LEFT JOIN courses ON student_courses.course_id = courses.id").
...
func GetStudentStatisticsByState(c *gin.Context) {
	...
	userRole, ok := userRoleStr.(int)
	if !ok {
		c.JSON(http.StatusUnauthorized, gin.H{"message": "unauthorized user failed to convert to int"})
		return
	}
	...
	case 2:
		// MDA user: filter by MDA ID
		var userMdaId uint
		err = config.DB.Table("mdas").
			Where("user_id = ?", userID).
			Pluck("id", &userMdaId).Error
		if err != nil {
			c.JSON(http.StatusUnauthorized, gin.H{"message": "MDA account has issues"})
			return
		}
	...
	case 3:
		...
		err = config.DB.Table("stcs").
			...
			Scan(&userStcId).Error
		if err != nil {
			c.JSON(http.StatusUnauthorized, gin.H{"message": "STC account has issues"})
			return
		}
