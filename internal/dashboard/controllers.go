...
func GetTopStc(c *gin.Context) {
	...
	case 1:
		err := config.DB.Table("stcs").
			Select("stcs.name as stc_name, stcs.id as stc_id, COUNT(students.id) as total_students, COALESCE(SUM(CASE WHEN student_courses.is_certified = true THEN 1 ELSE 0 END), 0) as total_certified").
